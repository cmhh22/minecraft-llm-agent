...
export class Coder {
    ...
    async  _lintCode(code) {
        ...
        // allDocs is an array of strings like "skills.placeBlock\n * doc..."
        // Extract valid function names from doc strings
        const validFunctions = new Set();
        for (const doc of allDocs) {
            const funcMatch = doc.match(/^(?:skills|world)\.([\w]+)/);
            if (funcMatch) validFunctions.add(funcMatch[1]);
        }
        // check function exists
        const missingSkills = usedSkills.filter(skill => !validFunctions.has(skill));
